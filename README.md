...
<!-- 录好录屏后，把下面这行换成：![任务做完，喇叭念一句；你说完，它自动发送](docs/demo.gif) -->

![任务做完，喇叭念一句；你说完，它自动发送](docs/demo-placeholder.svg)

**任务跑完 → 喇叭念一句凝练的结果 → 你随口说一句 → 自动转写并发送 → 念完回复，麦克风自己重新打开。** 全程不用碰键鼠。
（最容易被记住的一点：**念出来的是专门为耳朵写的一句，不是把正文读一遍**。）

> 想自己录一份换上去？分镜脚本在 [`docs/demo-script.md`](docs/demo-script.md)。
...
- 实时对话循环在 `client-smoke` 里全绿：静音不再中断对话（窗口翻页、麦克风全程不灭）、说话后静音自动结束并自动提交、
  念完回复自动重新开麦、取消的那轮也会重新开麦、关掉立刻释放麦克风；房间噪声与 400ms 短促声都不会被上传。
  这一路抓到六个真 bug：门限估计会被「开头就大声」的第一帧顶到说话声之上（循环直接变聋）；
  说话期间关掉开关时**还在打开的麦克风会留下来继续录**；状态机没有重入保护会无限递归；
  **房间噪声被当成说话上传、识别模型对近静音编出一句话、循环把它当用户的话发了出去（实测误发 12 次）**；
  以及静音 30 秒就释放麦克风，导致"说完一句、隔一会儿再说"完全用不了。
...
- **真人开口的完整一圈已经跑通**（部署侧的诊断数据，不是脚本）：`loudest` 0.128 / `speechFrames` 14
  的真实人声 → 自动转写 → 自动发送（`voice-submitted`）→ 播报 → 麦克风自己重新开麦，
  期间窗口翻页一次（`handsfree-roll`）且 `mic-idle-timeout` 为 0 —— 也就是说完停一会儿再继续说，不会再被断掉。
